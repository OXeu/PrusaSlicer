...
namespace Slic3r {
    ...
    public:
        LayerExtrusionRange(uint32_t iregion, ExtrusionRange extrusion_range) : m_region(iregion),
    ...
    public:
        ...
        void add_fill_range(const LayerExtrusionRange &new_fill_range) {
            // Compress ranges.
            if (!this->fills.empty() && this->fills.back().region() == new_fill_range.region() && *this->fills.back().
                end() == *new_fill_range.begin())
    ...
    public:
        ...
        std::shared_ptr<PrintObject> &object() { return m_object; }
        const std::shared_ptr<PrintObject> object() const { return m_object; }
    ...
    protected:
        ...
        Layer(size_t id, PrintObject *object, coordf_t height, coordf_t print_z,
              coordf_t slice_z) : upper_layer(nullptr), lower_layer(nullptr),
                                  //slicing_errors(false),
                                  slice_z(slice_z), print_z(print_z), height(height),
                                  m_id(id) {
            m_object = std::make_shared<PrintObject>(object->print(), object->model_object(), object->trafo(), object->instances());
        }
    ...
    private:
        ...
        size_t m_id;
        std::shared_ptr<PrintObject> m_object;
    ...
    protected:
        ...
        SupportLayer(size_t id, size_t interface_id, PrintObject *object, coordf_t height, coordf_t print_z,
                     coordf_t slice_z) : Layer(id, object, height, print_z, slice_z), m_interface_id(interface_id) {
        }
